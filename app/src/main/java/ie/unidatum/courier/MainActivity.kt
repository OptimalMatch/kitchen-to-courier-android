...
class MainActivity : AppCompatActivity() {
    ...
    private fun render(status: String, orders: List<JSONObject>, menu: Map<String, MenuItem>, hubSeen: String) = ui.post {
        ...
        findViewById<TextView>(R.id.position).text = "position ${"%.5f".format(courier.lat)}, ${"%.5f".format(courier.lon)} · ${courier.locationMode} · signed ${Keys.backing} · sent $age" + (courier.publishError?.let { " · $it" } ?: "")
